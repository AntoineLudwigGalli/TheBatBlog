...
class AppFixtures extends Fixture
{
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        $admin
            ->setEmail('admin@a.a')
            ->setRegistrationDate($faker->dateTimeBetween("17/04/1915", "now"))
            ->setPseudonym('Batman')
            ->setRoles(['ROLE_ADMIN'])
            ->setPassword(
                $this->encoder->hashPassword($admin, 'aA1!aaaa')
            )
        ;
        ...
        for($i=0; $i<10; $i++){
            ...
            $user
                ...
                ->setRegistrationDate($faker->dateTimeBetween("-2 years", "now"))
                ->setPseudonym($faker->userName)
                ->setPassword(
                    $this->encoder->hashPassword($user, 'aA1!aaaa')
                )
            ;
